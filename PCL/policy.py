...
class Policy(object):
  ...
  def core(self, obs, prev_internal_state, prev_actions):
    ...
    for i, (obs_dim, obs_type) in enumerate(zip(self.env_spec.obs_dims, self.env_spec.obs_types)):
    ...
    if self.input_prev_actions:
      if self.env_spec.combine_actions:  # TODO(ofir): clean this up
        ...
        for i, (act_dim, act_type) in enumerate(zip(self.env_spec.act_dims, self.env_spec.act_types)):
  ...
  def sample_actions(self, output, actions=None, greedy=False):
    ...
    #print(start_idx)
    #print(self.env_spec.total_sampling_act_dim)
  ...
  def single_step(self, prev, cur, greedy=False):
    ...
    prev_internal_state, prev_actions, _, _  = prev
    ...
    (actions, logits, log_probs) = self.sample_actions(
  ...
  def sample_step(self, obs, prev_internal_state, prev_actions, greedy=False):
    ...
    #, entropies, self_kls
    (next_state, sampled_actions, logits, log_probs) = self.single_step(
        (prev_internal_state, prev_actions, None, None),
  ...
  def multi_step(self, all_obs, initial_state, all_actions):
    ...
    # entropies, self_kls)
    (internal_states, _, logits, log_probs) = tf.scan(
        self.single_step,
        (all_obs, all_actions),
  ...
  def get_initializer(self, batch_size, initial_state, initial_actions):
    ...
    logits_init = []
    log_probs_init = []
    for act_dim, act_type in zip(self.env_spec.act_dims, self.env_spec.act_types):
      sampling_dim = self.env_spec.sampling_dim(act_dim, act_type)
...
class MLPPolicy(Policy):
  ...
  def single_step(self, obs, actions, prev_actions, greedy=False):
    ...
    return (next_state, tuple(actions), tuple(logits), tuple(log_probs)) #tuple(entropy), tuple(self_kl))

  def sample_step(self, obs, prev_internal_state, prev_actions, greedy=False):
    """Sample single step from policy."""
    # ,entropies, self_kls
    (next_state, sampled_actions, logits, log_probs) = self.single_step(obs, None, prev_actions,
                                             greedy=greedy)
    return next_state, sampled_actions
  ...
  def multi_step(self, all_obs, initial_state, all_actions):
    ...
    reshaped_obs = []
    for obs, (obs_dim, obs_type) in zip(all_obs, zip(self.env_spec.obs_dims, self.env_spec.obs_types)):
      if self.env_spec.is_discrete(obs_type):
    ...
    (internal_states, _, logits, log_probs) = self.single_step(
